...
function help_message() {
  ...
  -f|--father   Path to the father's bam file
  -m|--mother   Path to the mother's bam file
  -c|--child    Path to the child's bam file
...
while [ "$#" -gt 0 ]; do
  case "${1}" in
    ...
    -f|--father) FATHER="${2}"; shift 2;;
    -m|--mother) MOTHER="${2}"; shift 2;;
...
echo "new"
echo "genome ${GENOME}"
echo "load ${FATHER} index=${FATHER}.bai"
echo "load ${MOTHER} index=${MOTHER}.bai"
echo "load ${CHILD} index=${CHILD}.bai"
echo "snapshotDirectory ${OUTPUT_DIR}"
awk 'BEGIN{FS="\t"} {print "goto "$1":"$2"-"$3"\nsort base\nsquish\nsnapshot "$1"_"$2"_"$3"_"$4".png" }' "${BED4}"
